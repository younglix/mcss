...
export default function BursaryPayments() {
  ...
  return (
    <DashboardPageShell portalId="bursary" pageTitle="Payments" title="Payments" subtitle="Collect fees against an outstanding invoice, in any method." loading={loading} error={error} onReload={reload} skeletonCount={1}>
      {data && (
                    ...
                    <tbody className="divide-y divide-outline/10">
                      {payments.map((p) => (
                        <tr key={p.id}>
                          ...
                          <td className="px-lg py-3"><Badge tone={STATUS_TONE[p.status] || 'secondary'}>{p.status}</Badge></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
