...
function resolveHomePath({ permissions, roles, user, requestedFrom }) {
  ...
  if (user?.user_type === 'staff' && roles?.includes('exam_officer') && !roles?.includes('principal')) return '/staff/exam-officer/dashboard';
  if (user?.user_type === 'staff' && roles?.includes('principal')) return '/principal';
